...
use BenTools\SimpleDBAL\Model\Adapter\PDO\Statement;
use BenTools\SimpleDBAL\Model\Adapter\PDO\Result;

class PDOStatementTest extends PDOTestCase
{
    ...
    public function testPreparedWriteStmt()
    {
        ...
        $this->assertInstanceOf(\PDOStatement::class, $wrappedStatement);
        $this->assertEquals(1, $wrappedStatement->rowCount());
    ...
    public function testGetters()
    {
        $sql = sprintf("SELECT `name` FROM `%s` WHERE `name` = 'foo'", self::$tableName);
        $stmt = self::$cnx->prepare($sql);
        $this->assertInstanceOf(\PDOStatement::class, $stmt->getWrappedStatement());
